...
## v1.11.2 修正
- Editor起動時にIndexedDB / File System Access APIの確認が無応答でも停止しないよう修正。
- HTTP経由の3JSON読込を先行し、接続フォルダ確認は1.8秒でタイムアウトしてHTTPモードを継続します。
- HTTP読込にも4秒タイムアウトを設定し、「読み込み元を確認中...」の永久待機を防止しました。
- 保存済みフォルダ接続が正常なら、HTTP表示後に直接読込モードへ切り替わります。
